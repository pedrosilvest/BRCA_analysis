...
#create folders
logs_path=$scratch_path/output/logs/05_align_tolerant
scripts_path=$scratch_path/scripts/05_align_tolerant
...
apptainer exec "\$BWA" bwa mem -B 3 -O 5 -E 2 -L 5 -M -t 30 -R "\$RG" "\$GENOME" "\$R1" "\$R2" | \
...
mkdir -p $output_path/05_align_tolerant
mkdir -p $output_path/05_align_tolerant/\$sample

bam_dir="$output_path/05_align_tolerant/\$sample/bam/"
stats_dir="$output_path/05_align_tolerant/\$sample/stats/"
mkdir -p \$bam_dir
mkdir -p \$stats_dir
